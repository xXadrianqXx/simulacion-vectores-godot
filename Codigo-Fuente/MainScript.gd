...
#Pausar el Juego.
#Recibe la señal del botón paused
func _on_pause_pressed() -> void:
	$UI.visible = false #Oculta toda la UI.
	 #Muestra el menú de pausa
	$PausedUI.visible = true
	get_tree().paused = true #pausa todos los process



#señal recibida del botón éxit to Main menú.
func _on_button_exit_to_main_menu_pressed() -> void:
	get_tree().paused = false #Reanuda todos los process
	get_tree().change_scene_to_file("res://MenuScene.tscn")

#señal recibida del botón reboot
func _on_button_reboot_pressed() -> void:
	$PausedUI.visible = false#oculta el menú de pausa.
	$UI.visible = true #Muestra toda la UI.
	get_tree().paused = false #pausa todos los process
	_on_robot_reboot()


#señal recibida del buzón resume
