...
var muerte = false #Estado del Enemigo.
var atacando = false #Estado del enemigo
#rango_ataque = false #El robot está dentro del rango de ataque
var robot_e = null #Aqui se asigna el nodo Robot para usar su función damage_taken()
...
func _physics_process(delta: float) -> void:
	...
	#region Aqui se verifica si el Enemigo se encuentra dentro del rango permitido del mapa.
	...
	if vida.value == 0: #Si la vida es 0.
		deplazamiento = Vector2.ZERO #Hacemos que ya no cambie de posición  haciendo que el vector desplazamiento sea (0,0)
		animacion_enemigo.play("death")
		muerte = true #Actualiza el estado de muerte a verdadera

	#Calculo de la distacia que tienen el Robot y el Enemigo por medio de distance_to() 
	...
	#Si la distancia_futura es menor a el rango_maximo y no está muerto.
	if distancia_futura < rango_max and !muerte:
		atacar(robot)
	if atacando:
...
##Recibe daño
func ataque_recibido(damage: int):
	vida.value -= damage

##Atacar
func atacar(robot):
	atacando = true #Estado de ataque verdadero
	animacion_enemigo.play("attack")
	robot_e = robot #Asignamos el nodo Robot a la variable.

##Señal de Finalización de animación.
func _on_animaciones_animation_finished() -> void:
	if animacion_enemigo.animation == "death":
		queue_free()
		GlobalScript.change_score(1)
	
	if animacion_enemigo.animation == "attack":
		#si se encuentra el nodo robot
		if robot_e != null:
			#Calcular la distancia 
			var distancia_actual = position.distance_to(robot_e.position)
			if distancia_actual < 40:
				#Si está dentro del rango de ataque llamar a la función del robot damage Taken.
				robot_e.damage_taken(15)
		atacando = false   #volver al estado no Atacando
		robot_e = null #Volver a la normalidad a la variable(Evíta errores al volver atacar)
